...
class Telnet {
	 ...
	 * @return void
	 */
	public function __construct($host = '127.0.0.1', $port = '22', $timeout = 100, $prompt = '$', $stream_timeout = 1) {
		$this->host = $host;
		$this->port = $port;
		$this->timeout = $timeout;
		$this->setPrompt('Welcome to AscEmu\'s Remote Administration Console.');
        $this->setPrompt('Please authenticate to continue. ');
	...
	public function login($username, $password) {
		try {
			...
			$this->waitPrompt();
			$this->write($password);
            $this->setPrompt('Type ? to see commands, quit to end session.');
			$this->waitPrompt();
            $this->setPrompt();
		} catch (Exception $e) {
